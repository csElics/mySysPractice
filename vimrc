...
Plugin 'Lokaltog/vim-easymotion' "快捷移到光标
Plugin 'majutsushi/tagbar' "比如taglist好用
Plugin 'airblade/vim-gitgutter' "显示出gitdiff
"Plugin 'pangloss/vim-javascript' "js 缩进
"Plugin 'mattn/emmet-vim' "html 快速输入
Plugin 'tpope/vim-tbone' "tmux 工具 可以进行复制 粘贴
Plugin 'tpope/vim-repeat' "easyclip依赖
Plugin 'svermeulen/vim-easyclip' "更容易去copy
...
map <leader>d #*
" php error_log 方便高度
map <leader>k :set paste<cr>yawoerror_log(print_r("\n<esc>pa\=error::dfy===>>".__CLASS__."->".__FUNCTION__."<<========================>>>>> \n", true), 3, "/tmp/dfy.log");<ESC>oerror_log(print_r($<esc>pa, true), 3, "/tmp/dfy.log");<esc>,w:set nopaste<cr>
" 是否显示行号切换, 方便复制
